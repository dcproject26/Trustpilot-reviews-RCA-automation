...
def main(argv=None):
    ...
    try:
        ...
        if gaps is None:
            ...
            # "THE MODEL WAS ASKED" IS A CLAIM, and it was false three runs
            # running. An empty list written by a prompt that had no `gaps` in
            # its JSON skeleton is the model answering a question nobody put
            # to it, and it printed the same sentence as a genuinely clean
            # case. The stamp is the only thing that separates them.
            from server.prompts import prompt_stamp_state
            if prompt_stamp_state(d.rca_prompt_version) == "current":
                print("  Empty array, from THIS prompt build. The model was "
                      "asked and found no unsolved gap — a real answer.")
            else:
                print("  Empty array — but see the banner above. This draft "
                      "was NOT written by the running prompt, so the empty "
                      "list is 'not asked', not 'nothing found'. Regenerate "
                      "before reading it as this case's answer.")
